...
// if shouldUpsertPrice is set to true, we will try and tokenjob period[i], which will try to ensure period[i-1]
// if shouldUpsertPrice is set to false, we will not calculate a tokenjob and will upsert it to 0
...
// getVaultPeriodPriceBOverA calculate and return normalized tokenjob of b over a
...
func (p impl) getVaultPeriodPriceBOverA(ctx context.Context, periodI drip.VaultPeriod) (decimal.Decimal, error) {
	...
	// average tokenjob from period I to period I-1
...
// ensureVaultPeriod - if vaultPeriod exists return it , else upsert vaultPeriods with a tokenjob of 0
