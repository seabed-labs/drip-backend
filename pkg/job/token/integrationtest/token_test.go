	...
	t.Run("should update market tokenjob for 2 tokens", func(t *testing.T) {
		...
		integrationtest.InjectDependencies(
			...
			func(
				...
				assert.Equal(t, 0.913555, *orca.UIMarketPrice)
				assert.Equal(t, int32(673), *orca.MarketCapRank)

				assert.Equal(t, msolTokenAddress, msol.Pubkey)
				assert.Equal(t, int16(9), msol.Decimals)
				assert.Equal(t, "mSOL", *msol.Symbol)
				assert.Equal(t, "Marinade staked SOL", *msol.Name)
				...
				assert.Equal(t, 39.45, *msol.UIMarketPrice)
				assert.Equal(t, int32(147), *msol.MarketCapRank)
